...
class SDS011:
    ...
    def wake(self):
        """Sends wake command to  (starts its fan)."""
        cmd = self.make_command(_SDS011_CMDS["SLEEPWAKE"], _SDS011_CMDS["SET"], chr(1))
        self._uart.write(cmd)

    def sleep(self):
        """Sends sleep command to sds011 (stops its fan)."""
        cmd = self.make_command(_SDS011_CMDS["SLEEPWAKE"], _SDS011_CMDS["SET"], chr(0))
        self._uart.write(cmd)
    ...
    def process_measurement(self, packet):
        try:
            *data, checksum, tail = struct.unpack("<HHBBBs", packet)
            ...
            self._id2 = data[3]
            checksum_OK = (
                checksum == (sum(data) + (data[0] >> 8) + (data[1] >> 8)) % 256
            )
            tail_OK = tail == b"\xab"
